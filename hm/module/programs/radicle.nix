      ...
      uri = {
        ...
        web-rad = let
          ...
          in
            if detectionList == []
            then null
            else head detectionList;
        in {
          enable = mkEnableOption "`web+rad:`-URI handling by web browser";
          browser = mkOption {
            description = "Name of the XDG Desktop Entry for your browser. LibreWolf, Firefox and Chromium configured via home-manager will be detected automatically. The value of this option should likely be the same as the output of `xdg-mime query default x-scheme-handler/https`.";
            type = nullOr str;
  ...
  config = mkIf cfg.enable {
    ...
    xdg = {
      mimeApps.defaultApplications = {
        "x-scheme-handler/rad" = mkIf (cfg.uri.rad.browser.enable || cfg.uri.rad.vscode.enable) (mkDefault ("rad-to-"
          + (
            if cfg.uri.rad.browser.enable
            then "browser"
            else if cfg.uri.rad.vscode.enable
            then "vscode"
      ...
      in
        listToAttrs (map (v: {
          ...
          }) [
            {
              name = "Web Browser";
              shortName = "browser";
              prefix = replaceStrings ["$host" publicExplorerSuffix] [cfg.uri.rad.browser.preferredNode ""] cfg.settings.publicExplorer;
            }
            {
              name = "VSCode";
              shortName = "vscode";
