...
# and that a bob peer can send alice a patch via the seed.
{
  ...
  seed-ssh-keys = import "${nixpkgs}/nixos/tests/ssh-keys.nix" pkgs;
  seed-tls-certs = import "${nixpkgs}/nixos/tests/common/acme/server/snakeoil-certs.nix";
...
in {
  ...
  nodes = {
    ...
    bob = {
      imports = [
        commonHostConfig
        {services.radicle.package = pkgs."radicle-node-master";}
      ];
    };
  };
